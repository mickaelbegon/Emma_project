...
def save_sol_states_controls(sol, filename):
    ...
    taus = np.hstack([p["tau"] for p in sol.decision_controls(to_merge=[SolutionMerge.NODES])])
...
#         objective_functions.add(ObjectiveFcn.Lagrange.MINIMIZE_STATE, key="tau", quadratic=True, weight=weight_tau, phase=phase)
        ...
        objective_functions.add(ObjectiveFcn.Lagrange.MINIMIZE_CONTROL, key="tau", derivative=True, quadratic=True, weight=weight_tau/1000, phase=phase)
